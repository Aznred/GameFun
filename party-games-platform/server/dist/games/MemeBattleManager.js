...
class MemeBattleManager {
    ...
    submitCaption(playerId, caption) {
        if (this.state.phase !== 'writing')
            return;
        if (this.state.captions.some((c) => c.playerId === playerId))
            return;
    ...
    vote(voterId, targetPlayerId) {
        ...
        if (this.state.votes[voterId])
            return;
        if (!this.state.captions.some((c) => c.playerId === targetPlayerId))
            return;
...
exports.MemeBattleManager = MemeBattleManager;
